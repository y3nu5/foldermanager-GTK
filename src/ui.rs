...
use crate::scan::{format_bytes, parse_filter_option, scan_folder, FolderStats};
...
pub fn build_ui(app: &Application) {
    ...
    glib::source::timeout_add_local(Duration::from_millis(100), move || {
        match rx.try_recv() {
                ...
                match res {
                    Ok(stats) => {
                        ...
                        for (ext, cnt) in stats.extension_count.into_iter() {
                            ...
                            let label =
                                Label::new(Some(&format!("{} : {} file", ext, cnt)));
                    ...
                    Err(err) => {
                        ...
                        count_label_clone
                            .set_text(&format!("Total files: - (Error: {})", err));
                    }
                }
            ...
            Err(TryRecvError::Disconnected) => {
                ...
                spinner_clone.set_visible(false);
                count_label_clone
                    .set_text("Error: worker thread disconnected");
                ControlFlow::Break
            }
        }
    });
    ...
    calc_btn.connect_clicked(move |_| {
        ...
        if text.trim().is_empty() {
            ...
            count_label_calc
                .set_text("Total files: - (masukkan path dulu)");
            return;
        }
        ...
        if !pb.exists() || !pb.is_dir() {
            total_label_calc.set_text("Total size: -");
            count_label_calc
                .set_text("Total files: - (path tidak valid)");
            return;
        }
